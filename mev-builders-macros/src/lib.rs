...
pub fn include_builders(_input: TokenStream) -> TokenStream {
    ...
    let builders_json = fs::read_to_string(&builders_path)
        .expect(&format!("Failed to read builders.json from {:?}", builders_path));
    let stats_json = fs::read_to_string(&stats_path)
        .expect(&format!("Failed to read builders_stats.json from {:?}", stats_path));
    
    // Parse JSON using structs
    let builders_data: Vec<BuilderJson> = serde_json::from_str(&builders_json)
        .expect("Failed to parse builders.json");
    let stats_data: HashMap<String, u64> = serde_json::from_str(&stats_json)
        .expect("Failed to parse builders_stats.json");
    
    // Create a vec of (builder, blocks) tuples
    let mut builders_with_blocks: Vec<(BuilderJson, u64)> = builders_data
        .into_iter()
        .map(|builder| {
            let blocks = builder.extra_data.as_ref()
                .and_then(|extra_data| stats_data.get(extra_data))
                .copied()
                .unwrap_or(0);
            (builder, blocks)
        })
    ...
    let builder_tokens: Vec<_> = builders_with_blocks.iter().map(|(builder, blocks)| {
